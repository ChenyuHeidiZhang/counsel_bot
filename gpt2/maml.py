...
NUM_TEST_TASKS = 600  # TODO: update this
...
class Gpt2MAML:
    ...
    def _inner_loop(self, tokenized_seqs, train):
        ...
        """
        model = copy.deepcopy(model)  # does gradient flow through deepcopy?
        for _ in range(self._num_inner_steps):
            ...
            for i, param in enumerate(model.parameters()):
                param = param - self._inner_lr * d_loss[i]
        return model
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            inp_support = inp_support.to(DEVICE)
            out_support = out_support.to(DEVICE)
            inp_query = inp_query.to(DEVICE)
            out_query = out_query.to(DEVICE)
            ...
            if not train:  # do the evaluation only when not training
                input_ids = self._tokenizer(inp_query, return_tensors='pt').input_ids.to(DEVICE)
                sampled_tokens = model.generate(input_ids, max_length=MAX_TOKENS)
                decoded = self._tokenizer.decode(sampled_tokens).strip()
                score = utils.get_bleu(decoded, out_query)
                score_query_batch.append(score)

        outer_loss = torch.mean(torch.stack(outer_loss_batch))
        score_query = torch.mean(torch.stack(score_query_batch))
    ...
    def train(self, dataloader_train, dataloader_val, writer):
        ...
        for i_step, task_batch in enumerate(
            ...
            if i_step % VAL_INTERVAL == 0:
                ...
                writer.add_scalar('score/val', score, i_step)
