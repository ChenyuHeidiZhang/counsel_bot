...
def run_icl(k, n_val=128, postprocess=True):
    ...
    targets = []
    predictions = []
    pbar = tqdm.tqdm(list(range(int(n_val))))
    for row in pbar:
        ...
        if postprocess:
            # print(generation_output.choices[0].text)
            # generation_output = utils._postprocess_generations(generation_output)
            generation_output = utils._postprocess_generations(generation_output.choices[0].text)
